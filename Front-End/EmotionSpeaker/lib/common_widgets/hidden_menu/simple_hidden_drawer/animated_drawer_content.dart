...
class _AnimatedDrawerContentState extends State<AnimatedDrawerContent> {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      return AnimatedBuilder(
        ...
        builder: (_, child) {
          ...
          if (widget.enableRotateAnimation)
            rotateAngle = widget.typeOpen == TypeOpen.FROM_RIGHT
                ? (0.12 * animatePercent)
                : (-0.12 * animatePercent);
          ...
          return Transform(
            transform: new Matrix4.translationValues(
                slideAmount,
                widget.enableRotateAnimation ? (animatePercent * 35.0) : 0.0,
                0.0)
  ...
  List<BoxShadow> _getShadow() {
    if (widget.withShadow) {
      return [
        new BoxShadow(
          color: const Color(0x44000000).withOpacity(0.15),

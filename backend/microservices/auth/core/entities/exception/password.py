...
class PasswordMinLengthBeyondLimit(PasswordException):
    """Exception raised when password's length < password min length"""
    ...
    def __str__(self):
        if self.password:
            return f'"{self.password}" exceeded the allowed length ({USER_PASSWORD_MIN_LENGTH}).'
        return 'The password Length exceeded the allowed length.'
